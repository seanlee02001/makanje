...
export default function NewDishPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!dish) { setError('Could not create dish'); return }
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="glass-strong sticky top-0 z-20 px-4 pt-4 pb-3 flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-gray-900 dark:text-[#FED7AA] font-heading">New Dish</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 p-4 max-w-lg mx-auto w-full">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Button type="submit" loading={loading} className="flex-1">Save dish</Button>
        </div>
      </form>
    </div>
  )
